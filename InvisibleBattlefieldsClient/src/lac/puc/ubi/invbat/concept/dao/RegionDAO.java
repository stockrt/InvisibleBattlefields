...
public class RegionDAO extends BaseDAO {
	...
	private RegionData setRegionData(ResultSet rs) throws SQLException {
		RegionData temp = new RegionData();
		// pega todos os atributos da Region
		temp.setId(rs.getInt("id"));
		temp.setName(rs.getString("name"));
		temp.setStrPoints(rs.getString("points"));
		return temp;
	}
	public Vector<RegionData> buscarTodos() {
		...
		try {
			...
			while (rs.next()) {
				RegionData temp = setRegionData(rs);
				resultados.add(temp);
			}
			return resultados;
		} catch (SQLException e) {
